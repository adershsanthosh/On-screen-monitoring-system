...
class ExamActivitySerializer(serializers.ModelSerializer):
    """Serializer for ExamActivity events"""

    class Meta:
        model = ExamActivity
        fields = [
            'id', 'timestamp', 'student_id', 'exam_id',
            'event_type', 'details'
        ]
        read_only_fields = ['id', 'timestamp']
...
class ExamNotificationSerializer(serializers.ModelSerializer):
    """Serializer for ExamNotification model"""

    class Meta:
        model = ExamNotification
        fields = [
            'id', 'created_at', 'student_id', 'exam_id',
            'message', 'severity', 'active'
        ]
